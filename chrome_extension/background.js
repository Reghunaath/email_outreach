...
function buildPrompt(question, jobDescription, profile, remark) {
  const profileSection = profile
    ? `${PROFILE_BASE}\n\nADDITIONAL NOTES FROM THE APPLICANT:\n${profile}`
    : PROFILE_BASE;
  const lines = [
    'You are helping a job applicant fill out an application form.',
    'Write a first-person answer to the application question below, drawing on the',
    ...
    '  different length.',
    '- This applies to emails, cover letters, and answers to application questions alike.',
...
async function generate({ question, board, postingId, pageText, remark }) {
  ...
  if (!apiKey) {
    return { error: 'No API key set. Open the extension options and add your Gemini API key.' };
  }
  if (!question) {
  ...
  const body = {
    contents: [{ parts: [{ text: buildPrompt(question, jobDescription, profile, remark) }] }],
  };
  ...
  try {
    ...
    return { error: `Gemini returned an unreadable response (HTTP ${res.status}).` };
  }
